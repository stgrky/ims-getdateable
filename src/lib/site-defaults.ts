...
export const defaultHomePage: HomePage = {
  ...
  // Assessment section hidden for now (clients pulled it). Content lives on
  // the unlinked /assessment page if it needs to come back.
  missionHeading: "",
  missionIntro: "",
  missionPoints: [],
...
// Episodes come straight from the embedded Spotify show, so we don't seed
// individual episode docs. Kept as an empty default; the podcast page's
// "episode notes" section only renders if episodes exist in Sanity.
